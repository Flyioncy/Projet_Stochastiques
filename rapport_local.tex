...
\begin{equation}\label{eq:systeme}
  dV_t = V_t\left(a - b\,P_t + X_t\right)dt,
  \qquad
  dP_t = P_t\left(-c + d\,V_t\right)dt.
\end{equation}
...
Le fichier contient $2000$ points mais nous ne connaissions pas a priori le pas de temps de la discrétisation. Une idée a été d'exploiter le fait que l'équation des prédateurs ne contient aucun bruit puisque l'on a exactement $dP_t = P_t(-c + d\,V_t)\,dt$. En discrétisant, le pas de temps devrait alors vérifier

\begin{equation}\label{eq:dt}
  dt \approx \frac{P_{i+1} - P_i}{P_i\,(-c + d\,V_i)}.
\end{equation}

Nous avons donc moyenné cette quantité sur l'ensemble des points tout en écartant ceux où le dénominateur $-c + d\,V_i$ devient trop petit. On obtient ainsi $dt \approx 0{,}015$, ce qui donne une durée d'observation $\tau = 2000 \times 0{,}015 = 30$. Cela nous a permis de construire l'axe des temps puis de tracer les deux populations (figure~\ref{fig:series}).
...
Pour essayer de mesurer l'effet du virus, nous avons eu l'idée d'intégrer le modèle déterministe~\eqref{eq:base}, c'est-à-dire le même système lorsque l'on retire le terme $X_t$. Par identification avec le système fourni, on reconnaît les coefficients $a = \tfrac{2}{3}$, $b = \tfrac{4}{3}$, $c = 1$ et $d = 1$. Nous sommes partis des mêmes conditions initiales que les données, soit $V_0 = 1$ et $P_0 = 0{,}4$, puis avons intégré par un schéma d'Euler explicite sur la même grille de temps. La figure~\ref{fig:comparaison} superpose les deux trajectoires.
...
\begin{equation}\label{eq:disc}
  V_i - V_{i-1} \approx V_{i-1}\left(a - b\,P_{i-1} + X_{i-1}\right)dt.
\end{equation}

Tous les autres termes étant connus, il ne reste qu'à isoler le virus.

\begin{equation}\label{eq:xrec}
  X_{i-1} \approx \frac{V_i - V_{i-1}}{V_{i-1}\,dt} - a + b\,P_{i-1}.
\end{equation}
