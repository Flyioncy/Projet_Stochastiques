...
Deux candidats passent ce premier filtre, le brownien avec dérive et l'Ornstein-Uhlenbeck. Sur une fenêtre aussi courte nous ne distinguons d'ailleurs pas à l'œil une dérive lente d'un retour à la moyenne. Il reste à les mettre à l'épreuve.

\subsection*{Validation des candidats par simulation}

Pour départager les deux candidats, nous estimons les constantes de chaque modèle, nous en simulons des trajectoires de $X_t$, puis nous comparons par un test de Kolmogorov-Smirnov le $X$ simulé au $X$ reconstruit. Si un modèle décrit bien le virus, ses simulations doivent suivre la même loi et le test ne doit pas conduire à l'écarter. Nous travaillons directement sur $X_t$ car un modèle qui reproduit le virus reproduira aussi les populations qui en découlent.

\paragraph{Estimation des constantes.} Les constantes se lisent sur les différences $\Delta X_i = X_{i+1} - X_i$. En discrétisant l'Ornstein-Uhlenbeck $dX_t = \theta(\mu - X_t)\,dt + \sigma\,dB_t$, on obtient

\begin{equation}\label{eq:ou-disc}
  \Delta X_i \approx \theta\mu\,dt - \theta\,dt\,X_i + \sigma\,\Delta B_i,
\end{equation}

soit une relation linéaire entre $\Delta X_i$ et $X_i$. Une régression donne alors $\theta$ par la pente, $\mu$ par l'ordonnée à l'origine et $\sigma$ par l'écart-type des résidus, ce qui donne $\theta\approx0{,}07$, $\mu\approx-0{,}66$ et $\sigma\approx0{,}05$. Pour le brownien avec dérive $dX_t = \nu\,dt + \sigma\,dB_t$, la moyenne et l'écart-type des différences donnent directement $\nu = \overline{\Delta X}/dt \approx -0{,}02$ et $\sigma = \mathrm{std}(\Delta X)/\sqrt{dt} \approx 0{,}05$.

\paragraph{Simulation.} Nous simulons ces modèles à la manière du cours. On tire d'abord un mouvement brownien standard comme la somme cumulée d'incréments gaussiens indépendants de variance $dt$, puis on en déduit chaque trajectoire, par $X_t = \nu t + \sigma B_t$ pour la dérive et par un schéma d'Euler-Maruyama pour l'Ornstein-Uhlenbeck.

\paragraph{Test.} Pour chaque modèle nous répétons la simulation un grand nombre de fois et nous relevons la p-value du test de Kolmogorov-Smirnov entre le $X$ simulé et le $X$ reconstruit. La p-value moyenne vaut environ $3\cdot10^{-10}$ pour l'Ornstein-Uhlenbeck et $8\cdot10^{-15}$ pour le brownien avec dérive, très en dessous du seuil usuel de $0{,}05$.

À priori, ni l'Ornstein-Uhlenbeck ni le brownien avec dérive ne sont retenus. Ce résultat rejoint le fait que les différences ne sont pas gaussiennes, alors que ces deux modèles reposent sur un bruit gaussien. Le modèle du virus reste donc à trouver.
