...
\subsection*{Loi des différences}

Dans les processus stochastiques, les différences jouent un rôle central, et une première idée naturelle est de vérifier si elles sont gaussiennes. Nous les avons donc comparées à une loi normale construite à partir de leur propre moyenne et de leur propre écart-type. La figure~\ref{fig:hist} superpose l'histogramme et cette normale ajustée. La distribution observée est nettement plus piquée autour de zéro et présente des queues plus lourdes que la cloche gaussienne.
...
Un test de Kolmogorov-Smirnov, dont seule la p-value nous intéresse comme en cours et en TP, donne ici une valeur de l'ordre de $6\cdot10^{-14}$. Il est dès lors vraisemblablement plausible de ne pas retenir l'hypothèse de normalité des différences. Un seul test ne suffit cependant pas à trancher, surtout sur un jeu de données unique et restreint, et nous formulons donc cette conclusion avec prudence.
...
\begin{table}[ht]
  ...
  \begin{tabular}{llcl}
    ...
    Brownien géométrique (Black et Scholes) & reste de signe constant & $\times$ & $X_t$ devient négatif \\
