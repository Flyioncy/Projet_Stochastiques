...
\subsection*{Test de l'hypothèse Ornstein-Uhlenbeck}

Pour mettre l'Ornstein-Uhlenbeck à l'épreuve, nous avons estimé ses paramètres puis comparé le virus reconstruit à ce que le modèle prédit. L'équation discrétisée $\Delta X_i \approx \theta(\mu - X_i)\,dt + \sigma\sqrt{dt}\,\varepsilon_i$ se lit comme une régression de $\Delta X_i$ sur $X_i$, dont la pente donne $\theta$, l'ordonnée à l'origine donne $\mu$ et l'écart-type des résidus donne $\sigma$. On obtient $\theta \approx 0{,}07$, $\mu \approx -0{,}66$ et $\sigma \approx 0{,}05$.

Avec ces paramètres, un Ornstein-Uhlenbeck parti de $X_0 = 0$ a pour moyenne $\mu(1 - e^{-\theta t})$, une relaxation lente vers le niveau négatif $\mu$. La figure~\ref{fig:ou} compare le virus reconstruit à cette moyenne et à quelques trajectoires simulées du modèle. Le virus reconstruit suit la relaxation prévue et reste dans le faisceau des simulations.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.9\textwidth]{virus4_ou_test.png}
  \caption{Virus reconstruit $X_t$ comparé à la moyenne d'un Ornstein-Uhlenbeck $\mu(1-e^{-\theta t})$ (tirets) et à des trajectoires simulées du modèle (gris).}
  \label{fig:ou}
\end{figure}

À priori, l'Ornstein-Uhlenbeck reproduit l'allure observée. Il reste une approximation puisque les différences ne sont pas gaussiennes, et une seule trajectoire ne suffit pas à l'affirmer.
