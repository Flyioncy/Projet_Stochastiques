...
Cette reconstruction est entièrement déterministe puisqu'elle ne fait qu'inverser le schéma, sans aucune hypothèse de modèle. La figure~\ref{fig:xt} montre le virus reconstruit et ses incréments, et le tableau~\ref{tab:stats} en résume les statistiques.
...
\begin{table}[ht]
  ...
  \begin{tabular}{lc}
    ...
    \hline
    moyenne de $X_t$ & $-0{,}40$ \\
    écart-type de $X_t$ & $0{,}23$ \\
    moyenne de $\Delta X_t$ & $-3\cdot 10^{-4}$ \\
    coefficient de diffusion $\sigma = \mathrm{std}(\Delta X_t)/\sqrt{dt}$ & $0{,}05$ \\
    \hline
  \end{tabular}
  \caption{Statistiques empiriques du virus reconstruit $X_t$ et de ses incréments $\Delta X_t$. Pour les incréments, l'écart-type brut dépend du pas $dt$, on lui préfère le coefficient de diffusion $\sigma$.}
  \label{tab:stats}
\end{table}

Sa moyenne empirique vaut environ $-0{,}40$, donc négative, ce qui irait dans le sens de notre hypothèse sur l'équilibre. Avec une seule trajectoire de $2000$ points il faut toutefois rester prudent et se contenter de dégager des tendances. Le virus semblerait partir de $X_0 \approx 0$ puis décroître jusque vers $-0{,}6$ aux alentours de $t \approx 13$ avant de fluctuer autour de ce niveau. Ses incréments $\Delta X_t$ ont une moyenne quasi nulle et un coefficient de diffusion $\sigma \approx 0{,}05$, ce qui leur donne l'allure d'un bruit centré de petite amplitude.

\subsection*{Loi des incréments}

Pour préciser la loi de ces incréments, nous les avons comparés à une loi normale construite à partir de leur propre moyenne et de leur propre écart-type empiriques. La figure~\ref{fig:hist} superpose l'histogramme des incréments et cette normale ajustée. La distribution observée est nettement plus piquée autour de zéro et présente des queues plus lourdes que la cloche gaussienne, le signe que les variations sont le plus souvent minuscules mais entrecoupées de quelques sauts plus marqués.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.75\textwidth]{virus4_dXt_hist.png}
  \caption{Distribution des incréments $\Delta X_t$ comparée à une loi normale de mêmes moyenne et écart-type. La forme est plus piquée et à queues plus lourdes qu'une gaussienne.}
  \label{fig:hist}
\end{figure}

Un test de Kolmogorov-Smirnov confirme cette impression. En comparant les incréments à la normale ajustée, on obtient une statistique $D \approx 0{,}09$ et une p-value de l'ordre de $6\cdot10^{-14}$, très inférieure au seuil usuel de $0{,}05$. L'hypothèse de normalité est donc rejetée sans ambiguïté, ce qui nous rappelle qu'un modèle à bruit gaussien ne restera au mieux qu'une approximation.

\subsection*{Quel modèle pour $X_t$ ?}

Forts de ces observations, nous avons passé en revue les modèles vus en cours pour juger de leur pertinence. Le tableau~\ref{tab:modeles} les reprend dans l'ordre du cours, avec pour chacun la raison de le retenir ou de l'écarter.

\begin{table}[ht]
  \centering
  \begin{tabular}{p{2.8cm} p{4cm} c p{4.8cm}}
    \hline
    Modèle & Caractéristiques & Retenu & Justification \\
    \hline
    Brownien avec dérive & tendance linéaire sans borne & $\times$ & ne plafonne pas, or $X_t$ se stabilise \\
    Brownien géométrique (Black et Scholes) & reste toujours de signe constant & $\times$ & $X_t$ passe nettement sous zéro \\
    Pont brownien & épinglé à ses deux extrémités & $\times$ & $X_t$ ne revient pas à sa valeur initiale \\
    Brownien intégral & trajectoire très lisse & $\times$ & les incréments observés sont bruités \\
    Ornstein-Uhlenbeck & retour à la moyenne vers un niveau & $\checkmark$ & compatible avec la relaxation vers un niveau négatif \\
    Verhulst (logistique) & saturation d'une population positive & $\times$ & cadre positif mal adapté à un virus négatif \\
    \hline
  \end{tabular}
