...
Avant de fixer une équation pour $(X_t)$, nous avons passé en revue les modèles vus en cours pour voir lesquels pouvaient coller. Trois se laissent écarter d'emblée. Le mouvement brownien géométrique, qui sous-tend aussi le modèle de Black et Scholes, garde toujours un signe constant alors que notre virus passe nettement sous zéro. Le pont brownien est épinglé à une valeur fixe en fin d'intervalle, ce que la trajectoire reconstruite ne fait pas. Le brownien intégral donnerait quant à lui une trajectoire très lisse, bien plus régulière que les incréments bruités observés.

Restent les modèles à tendance ou à retour à la moyenne. Une simple dérive constante paraît peu compatible avec le plateau observé puisqu'elle continuerait de pousser le virus vers le bas sans jamais le stabiliser. Un processus d'Ornstein-Uhlenbeck, qui ramène doucement le virus vers un niveau d'équilibre négatif, nous a semblé le candidat le plus naturel. Sa version stationnaire est en revanche peu vraisemblable puisque le virus part de $0$ et semble encore relaxer vers ce niveau, ce qui évoque plutôt un régime transitoire. Le modèle de Verhulst, pensé pour une population positive qui sature, s'adapte mal à un virus qui devient négatif.

La distribution des incréments apporte toutefois une nuance. Elle est plus piquée qu'une loi normale et possède des queues plus lourdes (figure~\ref{fig:hist}). Aucun de ces modèles à bruit gaussien ne décrirait donc parfaitement $X_t$. Sur une seule trajectoire de $2000$ points nous préférons rester au stade de l'intuition, en retenant l'Ornstein-Uhlenbeck comme piste principale sans l'affirmer.
