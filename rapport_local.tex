...
\subsection*{Validation par simulation du système couplé}

Pour valider un modèle de $X_t$, nous simulons une trajectoire du modèle, nous l'injectons dans le système~\eqref{eq:systeme} pour en déduire $(V,P)$, puis nous comparons ces $(V,P)$ aux données par un test de Kolmogorov-Smirnov à deux échantillons. L'Ornstein-Uhlenbeck est estimé par la régression des différences sur le niveau ($\theta\approx0{,}07$, $\mu\approx-0{,}66$, $\sigma\approx0{,}05$) et le brownien avec dérive par la moyenne et l'écart-type des différences ($\nu\approx-0{,}02$, $\sigma\approx0{,}05$).

\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{virus4_vp_validation.png}
  \caption{Données observées (en couleur) et trajectoires $(V,P)$ simulées (en gris) à partir de chaque modèle de $X_t$, l'Ornstein-Uhlenbeck à gauche et le brownien avec dérive à droite.}
  \label{fig:val}
\end{figure}

L'écart est net sur la figure~\ref{fig:val}. Les prédateurs observés s'effondrent puis restent bas alors que les trajectoires simulées rebondissent et oscillent largement. Le test confirme ce constat avec des p-values de l'ordre de $10^{-50}$ pour les deux modèles, sur $V$ comme sur $P$.

À priori, ni l'Ornstein-Uhlenbeck ni le brownien avec dérive ne sont donc retenus. Un bruit gaussien stationnaire, qu'il revienne à la moyenne ou non, ne reproduit pas l'effondrement observé, ce qui rejoint le fait que les différences ne sont pas gaussiennes. Le modèle du virus reste donc à trouver.
