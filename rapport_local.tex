...
Deux candidats passent ce premier filtre, le brownien avec dérive et l'Ornstein-Uhlenbeck. Sur une fenêtre aussi courte nous ne distinguons d'ailleurs pas à l'œil une dérive lente d'un retour à la moyenne. Pour les départager, nous revenons à ce que chaque modèle affirme. Une équation $dX_t = a(X_t)\,dt + \sigma\,dB_t$ suppose qu'une fois la dérive $a(X_t)\,dt$ retirée il ne reste qu'un bruit brownien. En discrétisant, ses résidus $\varepsilon_i = \Delta X_i - a(X_i)\,dt$ doivent donc être indépendants et suivre une loi normale centrée d'écart-type $\sigma\sqrt{dt}$, puisque les incréments d'un mouvement brownien le sont. Tester un modèle revient alors à confronter ses résidus à ces deux exigences, leur loi et leur indépendance.
...
La dérive étant constante, les résidus $\varepsilon_i = \Delta X_i - \nu\,dt$ ne sont que les différences recentrées. Leur loi est donc celle déjà observée à la figure~\ref{fig:hist}, et un test de Kolmogorov-Smirnov contre la loi normale attendue donne une p-value de l'ordre de $10^{-14}$. Les résidus consécutifs gardent par ailleurs une corrélation de l'ordre de $0{,}26$, loin de l'indépendance d'un bruit brownien.
...
Les résidus $\varepsilon_i = \Delta X_i - \theta(\mu - X_i)\,dt$ devraient former le même bruit brownien. Le terme de rappel retiré étant minime, ils ne s'écartent pas davantage de la normale que ceux de la dérive, avec une p-value du test de Kolmogorov-Smirnov de l'ordre de $10^{-14}$, et ils conservent la même corrélation d'un pas à l'autre, de l'ordre de $0{,}26$.

\textbf{À priori, l'Ornstein-Uhlenbeck n'est pas retenu.}

\subsection*{Bilan}

Les deux candidats sont écartés pour la même raison. Tous deux reposent sur un bruit brownien, donc sur des résidus gaussiens et indépendants, alors que ceux du virus reconstruit s'écartent nettement d'une loi normale et restent corrélés d'un pas à l'autre. Le modèle du virus reste donc à trouver.
