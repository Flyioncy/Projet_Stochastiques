...
une relation linéaire entre $\Delta X_i$ et $X_i$. En faisant la régression linéaire de $\Delta X_i$ sur $X_i$, on trouve :
...
où l'intégrale d'Itô est approchée par une somme sur les incréments du brownien.

Le test de Kolmogorov-Smirnov entre le $X$ simulé et le $X$ reconstruit donne une p-value moyenne de l'ordre de $10^{-7}$ sur de nombreuses simulations.

Cette p-value est très inférieure à $0{,}05$. À priori, l'Ornstein-Uhlenbeck n'est pas retenu.

\subsection*{Test du brownien avec dérive}

Le brownien avec dérive s'écrit $dX_t = \nu\,dt + \sigma\,dB_t$, de solution immédiate $X_t = \nu\,t + \sigma\,B_t$. En le discrétisant, $\Delta X_i = \nu\,dt + \sigma\,\Delta B_i$, de sorte que la moyenne et l'écart-type des différences donnent

\begin{center}
  $\nu = \overline{\Delta X}/dt \approx -0{,}02, \qquad \sigma = \mathrm{std}(\Delta X)/\sqrt{dt} \approx 0{,}05.$
\end{center}

On simule $X_t = \nu\,t + \sigma\,B_t$ à partir du même brownien.

Le test de Kolmogorov-Smirnov donne une p-value moyenne de l'ordre de $10^{-9}$.

Cette p-value est elle aussi très inférieure à $0{,}05$. À priori, le brownien avec dérive n'est pas retenu.
