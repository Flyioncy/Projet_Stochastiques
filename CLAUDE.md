...
**Sur les tests statistiques :** le professeur insiste fortement sur leur interprétation, à respecter dans le rapport. Un test (Kolmogorov-Smirnov, etc.) ne *rejette* jamais une hypothèse au sens où il prouverait qu'elle est fausse. Comme en cours et en TP, on ne regarde que la **p-value**. Une p-value faible indique qu'il est *vraisemblablement plausible de ne pas retenir* l'hypothèse, ce qui n'est pas la même chose que de la déclarer rejetée. On reste d'autant plus prudent qu'un seul test, surtout sur un jeu de données unique et restreint, ne suffit jamais à conclure. Ces conclusions se formulent donc au conditionnel. On reste dans le cadre du cours et seule la p-value du test nous sert, sans statistique annexe comme une distance calibrée ou un Monte-Carlo sur une statistique de test. On garde aussi un esprit critique, car une p-value faible invite à changer d'hypothèse plutôt qu'à chercher à sauver le modèle (pas de biais de confirmation). Pour mettre un modèle d'EDS à l'épreuve, on ne compare pas deux trajectoires par un test à deux échantillons — les valeurs d'une trajectoire ne sont pas indépendantes, le test sur-rejette — mais on teste ses **résidus**, qui doivent être un bruit blanc gaussien (voir plus bas).
...
- Le test d'un modèle se lit sur ses résidus standardisés : on montre leur histogramme comparé à $\mathcal N(0,1)$ (la loi) et on donne la corrélation entre résidus consécutifs (l'indépendance), sans interpréter une trajectoire à l'œil.
...
### Tester un modèle par ses résidus

Un modèle $dX = a(X)\,dt + \sigma\,dB$ affirme qu'une fois la dérive retirée, le bruit est blanc et gaussien. En discrétisant, le résidu standardisé $\varepsilon_i = (\Delta X_i - a(X_i)\,dt)/(\sigma\sqrt{dt}) = \Delta B_i/\sqrt{dt}$ n'est qu'un incrément de mouvement brownien réduit, il doit donc suivre $\mathcal N(0,1)$ et être indépendant. On teste donc un modèle par ses résidus, sur deux points : leur **loi** (histogramme comparé à $\mathcal N(0,1)$ et KS à un échantillon, on ne lit que la p-value, jamais la distance $D$) et leur **indépendance** (corrélation entre résidus consécutifs proche de $0$). C'est le bon objet, car sous le modèle les $\varepsilon_i$ sont i.i.d., contrairement aux niveaux d'une trajectoire ; on ne compare donc jamais deux trajectoires par un KS à deux échantillons. La standardisation à $\mathcal N(0,1)$ ne change pas la forme mais rend la comparaison plus propre. Une fois la loi et l'indépendance acceptées, le modèle est cohérent et on le confronte au phénomène en injectant $X$ simulé dans $(V,P)$ pour vérifier par Monte-Carlo les faits observés (effondrement, plateau, probabilités d'extinction). Dans le rapport, on présente ce principe dans une section dédiée, avec les EDS des candidats et la forme générale $a(X_t)$, placée avant les sous-sections de test ; chaque sous-section se limite alors à l'estimation et au diagnostic, sans réécrire l'EDS.
