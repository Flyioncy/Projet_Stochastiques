...
- Le test d'un modèle se lit sur ses résidus : on peut montrer l'histogramme des résidus comparé à la loi normale attendue (la loi) et donner la corrélation entre résidus consécutifs (l'indépendance), sans interpréter une trajectoire à l'œil.
...
Un modèle $dX = a(X)\,dt + \sigma\,dB$ affirme qu'une fois la dérive retirée, le bruit est blanc et gaussien. En discrétisant, ses résidus $\varepsilon_i = \Delta X_i - a(X_i)\,dt$ doivent être indépendants et de loi $\mathcal N(0,\sigma^2 dt)$, puisque les incréments d'un brownien le sont. On teste donc un modèle par ses résidus, sur deux points : leur **loi** (KS à un échantillon contre la normale attendue, on ne lit que la p-value) et leur **indépendance** (corrélation entre résidus consécutifs proche de $0$). C'est le bon objet, car sous le modèle les $\varepsilon_i$ sont i.i.d., contrairement aux niveaux d'une trajectoire ; on ne compare donc jamais deux trajectoires par un KS à deux échantillons. Si $\sigma$ est constant, inutile de standardiser les résidus à variance $1$, la forme suffit. Une fois la loi et l'indépendance acceptées, le modèle est cohérent et on le confronte au phénomène en injectant $X$ simulé dans $(V,P)$ pour vérifier par Monte-Carlo les faits observés (effondrement, plateau, probabilités d'extinction).
