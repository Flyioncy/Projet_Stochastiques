...
Pour aller plus loin nous avons eu l'idée de remonter directement au virus à partir des données. L'équation des proies ne comporte aucun terme brownien, donc nous pouvons la discrétiser simplement dans le même esprit que la récupération de $dt$. En remplaçant $dV_t$ par l'incrément $V_i - V_{i-1}$ et en évaluant le membre de droite au début du pas, on obtient

\begin{equation}\label{eq:disc}
  V_i - V_{i-1} \approx V_{i-1}\left(\tfrac{2}{3} - \tfrac{4}{3}P_{i-1} + X_{i-1}\right)dt.
\end{equation}

Il ne reste plus qu'à isoler le virus, dont tous les autres termes sont connus.

\begin{equation}\label{eq:xrec}
  X_{i-1} \approx \frac{V_i - V_{i-1}}{V_{i-1}\,dt} - \tfrac{2}{3} + \tfrac{4}{3}P_{i-1}.
\end{equation}

Cette reconstruction est entièrement déterministe puisqu'elle ne fait qu'inverser le schéma discrétisé, sans aucune hypothèse de modèle. Nous l'avons sauvegardée dans le fichier \texttt{virus4\_Xt.csv} pour que chacun puisse repartir de la même série. La figure~\ref{fig:xt} montre le virus reconstruit et ses incréments.
...
Le virus part de $X_0 \approx 0$ puis décroît régulièrement jusque vers $-0{,}6$ aux alentours de $t \approx 13$, où il se stabilise en fluctuant. Sa moyenne empirique vaut environ $-0{,}40$, donc négative comme nous l'attendions. Un virus de moyenne négative abaisse en effet le taux de croissance des proies et décale l'équilibre des prédateurs vers $P^* = \tfrac{1}{2} + \tfrac{3}{4}\mathbb{E}[X]$, ce qui éclaire leur effondrement. Le scénario est même plus riche que prévu puisque l'intensité du virus ne reste pas constante mais se renforce au cours du temps, étouffant progressivement les prédateurs.

Les incréments $\Delta X_t$, eux, ressemblent à un bruit centré d'amplitude à peu près constante, sans tendance visible. Le virus lui-même paraît donc non stationnaire sur la fenêtre observée, alors que ses variations le sont. Pour préciser sa structure nous avons regardé son autocorrélation (figure~\ref{fig:xtacf}).
